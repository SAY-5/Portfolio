...
export const projects: Project[] = [
  ...
  },
  {
    ...
    tagline: `Scan sequencer that verifies bed and detector state before every capture`,
    summary: `scan-sequencer runs a small-animal PET scan as a state machine with one rule: never start until the hardware is verified in a known-good state, and never record a slice until the bed has physically arrived. The open-loop control before it fired each command and assumed it worked, losing about 10 runs a week to leftover bed positions, stale output-file locks, and captures taken mid-travel. Each run homes the bed to true mechanical zero via the limit switch, clears leftover locks, checks the detector, and confirms arrival before every capture. Everything runs against a simulated scanner on an injected virtual clock with a seeded fault stream, so runs are deterministic and no hardware is involved.`,
    category: "Instrumentation and Test",
    language: "Go",
    stack: ["Go", "Docker"],
    highlights: [
      `The backtest replays one seeded week, naive vs verified: failed runs drop from 10 a week to 3, all 7 preventable failures gone, only 3 injected hardware faults left.`,
      `Homing to true mechanical zero via the limit switch beats controller coordinate drift, so a bed position left over from the last run cannot poison this one.`,
      `Confirm-before-capture polls for arrival at each 10mm slice position before recording, which removes the mid-travel captures that logged data at the wrong position.`,
      `Fault injection flags (-leftover-pos, -lock-file, -detector-slow, -drift) reproduce each historical failure on demand, and time comes from an injected virtual clock so the suite runs under go test -race.`,
    ],
    demoConcept: `The same faulty job replayed side by side in naive and verified modes: naive fires commands blind and captures at the wrong position, verified homes to the limit switch, confirms each slice, and passes.`,
    flagshipScore: 9,
    isFlagship: true,
  },
  {
    ...
    tagline: `SCPI preflight checklist over TCP/IP that clears a PET scanner in seconds`,
    summary: `scan-preflight replaces the 13-step morning checklist for a networked PET scanner that someone used to read off by hand. It opens a TCP socket to the instrument, talks SCPI, runs the eight data checks, and prints one pass/fail summary; the five physical steps stay on paper as a printed reminder. Every check runs and everything is reported at the end instead of stopping at the first failure, and it exits non-zero on any failure so a downstream start-scan step can gate on it. A simulated instrument ships with the tool, so the checklist runs with no scanner attached; pointing it at real hardware means dropping the --sim flag.`,
    category: "Instrumentation and Test",
    language: "Python",
    ...
    highlights: [
      ...
      `The calibration-age check enforces a 30-day recalibration rule from the last_calibrated stamp, the kind of silent staleness a person reading the sheet skips right past.`,
      `Morning checklist: 15 to 20 minutes of careful reading, down to about 8 seconds against the real instrument.`,
      `Fault-injection flags (--low-disk, --bad-channel, --fail-selftest, --expire-calibration) force individual failures, so the report and the non-zero exit path get tested end to end.`,
    ],
    demoConcept: `A checklist panel connecting to a simulated instrument, lighting up all eight checks as their SCPI queries return, then a single cleared-for-scan verdict with the five manual steps listed as a reminder.`,
    flagshipScore: 7,
    isFlagship: false,
  },
  {
    ...
    tagline: `Reproduces and fixes the three recurring failure modes of shared lab benches`,
    summary: `A reproducible model of how a bank of shared lab workstations with serial peripherals was made to stop breaking. Three root causes were generating about eight support tickets a week: Windows Update silently swapping out the vendor serial driver, Linux renaming USB serial ports by plug order, and a comms timeout shorter than the device's real reply time. Each cause and its fix lives in code: a comms client with a measured timeout and retries, a udev rule generator that maps burned-in device serials to permanent names, and a ticket root-cause analyzer. The serial device is simulated behind a seeded, deterministic latency model on an injectable clock, so nothing here needs hardware.`,
    category: "Instrumentation and Test",
    language: "Go",
    stack: ["Go"],
    highlights: [
      `comms-bench, 200 queries with 600 to 850 ms replies: old 500 ms zero-retry policy 0%, measured 2000 ms plus 2-retry policy 100%. The timeout is deliberately not 30 s, so real failures still surface fast.`,
      `gen-udev turns a serial-to-name mapping into stable udev rules (ATTRS{serial} to SYMLINK), so software finds /dev/physio_monitor whatever the plug order. It rejects duplicate serials, duplicate names, and bad symlink names.`,
      `The tickets command builds a root-cause histogram and shows before and after: about eight tickets a week down to about two a month once the three causes were removed.`,
    ...
    demoConcept: `A strip chart of query times between 600 and 850 ms with the 500 ms timeout line cutting them off, then the line slides to 2000 ms and success flips from 0 to 100 percent.`,
    flagshipScore: 7,
    isFlagship: false,
  },
  {
